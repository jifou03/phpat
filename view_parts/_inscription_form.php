...
if (array_key_exists('prenom', $_POST)) {
    ...
    $prenom_ok = (1 === preg_match('/^[A-Za-zç\-]{2,}$/', $prenom));
...
if (array_key_exists('pseudo', $_POST)) {
    ...
    if ($pseudo_ok){
        //est ce que le pseudo est libre
        require_once 'db/_user.php';
        if (username_exists($pseudo)){
            echo "Le username" . $pseudo . "est déja utilisée";
        }
    }
    var_dump($pseudo);
    var_dump($pseudo_ok);
...
<form id="inscription" name="inscription" xmlns="http://www.w3.org/1999/html" method="post">
    ...
    <input type="password" name="mot_passe" id="mot_passe" value="<?php echo array_key_exists('mot_passe', $_POST) ? $_POST['mot_passe'] : '' ?>"/>
